...
export default async function NewPropertyPage({ searchParams }: Props) {
  ...
  return <><Header/><main className="account-page"><div className="container form-page"><div className="account-heading"><span className="section-kicker">Novo anúncio</span><h1>Publicar imóvel</h1><p>Informe os dados principais. Fotos e endereço detalhado poderão ser adicionados na edição.</p></div>{params.erro && <p className="form-alert">Revise os campos informados.</p>}<form className="listing-form" action={createProperty}>
    ...
    <label><span>WhatsApp do anúncio</span><PhoneInput defaultValue={user.phone || ""}/></label>

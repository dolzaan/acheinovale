...
import { slugify, uniqueSlug } from "@/lib/validation/listing";
...
export async function saveFreighterProfile(formData: FormData) {
  ...
  await prisma.$transaction(async tx => {
    const profile = existing ? await tx.freighterProfile.update({ where: { id: existing.id }, data: { displayName, description, cityId, whatsapp, serviceRadiusKm: Number(field(formData, "serviceRadiusKm")) || null, priceNote: field(formData, "priceNote") || null, availableToday: field(formData, "availableToday") === "on", status: "PENDING" } }) : await tx.freighterProfile.create({ data: { userId: user.id, slug: uniqueSlug(displayName), displayName, description, cityId, whatsapp, serviceRadiusKm: Number(field(formData, "serviceRadiusKm")) || null, priceNote: field(formData, "priceNote") || null, availableToday: field(formData, "availableToday") === "on", status: "PENDING" } });

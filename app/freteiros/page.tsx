...
type Props = { searchParams: Promise<{ q?: string; cidade?: string; pagina?: string }>; prettyPath?: boolean };
...
export default async function FreightersPage({ searchParams, prettyPath = false }: Props) {
  const params = await searchParams;
  const query = params.q?.trim().slice(0, 80) || "";
  const requestedPage = parsePage(params.pagina);
  if (!prettyPath && params.cidade) redirect(freighterPageHref(params.cidade, query, requestedPage));
  ...
  return <><Header citySlug={selectedCity?.slug}/><main className="catalog-page"><div className="container catalog-container">
    ...
    <Form className="catalog-filter catalog-filter--freighters" action={`/${selectedCity.slug}/freteiros`}><label className="catalog-filter__search"><SearchIcon size={19}/><input name="q" defaultValue={query} placeholder="Ex: mudança, entrega ou bairro" aria-label="Buscar freteiros"/></label><PendingSubmitButton className="button button--primary" pendingText="Buscando..." navigation>Buscar</PendingSubmitButton></Form>

...
type Props = { searchParams: Promise<{ q?: string; disponivel?: string; cidade?: string }> };

export default async function FreightersPage({ searchParams }: Props) {
  const params = await searchParams;
  const query = params.q?.trim().slice(0, 80) || "";
  const availableToday = params.disponivel === "hoje";
  ...
  const freighters = await prisma.freighterProfile.findMany({
    where: { status: "ACTIVE", city: { id: selectedCity?.id, isActive: true }, ...(availableToday ? { availableToday: true } : {}), ...(query ? { OR: [{ displayName: { contains: query, mode: "insensitive" } }, { description: { contains: query, mode: "insensitive" } }, { services: { some: { name: { contains: query, mode: "insensitive" } } } }] } : {}) },
  ...
  return <><Header citySlug={selectedCity?.slug}/><main className="catalog-page"><div className="container catalog-container">
    <div className="catalog-heading"><div><span className="section-kicker section-kicker--orange">Profissionais de {cityName}</span><h1>Freteiros</h1><p>Encontre ajuda para mudanças, entregas e transportes em {cityName} e região.</p></div><Link className="button button--primary" href="/publicar/frete">Divulgar meus fretes</Link></div>
    <Form className="catalog-filter" action="/freteiros">{selectedCity ? <input type="hidden" name="cidade" value={selectedCity.slug}/> : null}<label className="catalog-filter__search"><SearchIcon size={19}/><input name="q" defaultValue={query} placeholder="Ex: mudança, entrega ou bairro" aria-label="Buscar freteiros"/></label><label className="catalog-filter__check"><input type="checkbox" name="disponivel" value="hoje" defaultChecked={availableToday}/><span>Disponível hoje</span></label><PendingSubmitButton className="button button--primary" pendingText="Buscando..." navigation>Buscar</PendingSubmitButton></Form>
